 ...
 * Description:       Extended version of the very easy-to-use schedule management system to create beautiful responsive Timetable. Please update manually by dropping files from the .zip you downloaded on stereonomy.com into the plugin folder
 * Version:           1.3.9
...
function syet_save_planning() {
	...
	$save = Easy_Timetable_Admin::syet_saveTimetable($_POST);
	return $save;
}
...
function syet_edit_planning() {
	...
	require_once SYET_PATH . 'admin/class-easy-timetable-admin.php';
	$save = Easy_Timetable_Admin::syet_editPlanning($_POST);
	//var_dump($save);
	return $save;
}
...
function syet_delete_planning() {
	...
	require_once SYET_PATH . 'admin/class-easy-timetable-admin.php';
	$delete = Easy_Timetable_Admin::syet_deletePlanning($_POST);
	//var_dump($save);
	return $delete;
}

function syet_duplicate_planning() {
	$data = $_POST;	
	//Validating 
	if(isset($data["id"]))
	{
    ...
    //var_dump(stripslashes(stripslashes($data["activities"])));// Validate json data activities
    $arrayAct = explode(",{", stripslashes(stripslashes($data["activities"])));
    foreach ($arrayAct as $actjson) 
    {
    	if (preg_match('/^"a/', $actjson))
    	{
    ...
    if ($isJson == TRUE)
    {
    	$data["activities"] = stripslashes(stripslashes($data["activities"]));
    }
    else 
    {
    	$data["activities"] = "";
	...
	// Validate json data scheduledact
	$arrayPlanAct = explode(",{", stripslashes(stripslashes($data["scheduledact"])));
	foreach ($arrayPlanAct as $planactjson) 
	{
		//var_dump($planactjson);
		if(preg_match('/^"d/', $planactjson))
	...
	if ($isJsonP == TRUE)
    {
    	$data["scheduledact"] = stripslashes(stripslashes($data["scheduledact"]));
    ...
    //var_dump($data["days"]);
	$data["days"] = stripslashes(stripslashes($data["days"]));
	//var_dump($data["days"]);
	//Sanitazing data
	$data["action"] = sanitize_post_field('action', $data['action'], $data['id'], 'db');
	if (isset($data['id'])) {
	...
	require_once SYET_PATH . 'admin/class-easy-timetable-admin.php';
	$duplicate = Easy_Timetable_Admin::syet_duplicatePlanning($data);
	//var_dump($save);
	return $duplicate;
}

function save_refresh_planning() {
	require_once SYET_PATH . 'admin/class-easy-timetable-admin.php';
	$save = Easy_Timetable_Admin::syet_editPlanning($_POST);
	var_dump($save);
	return $save;
...
add_action('wp_ajax_syet_duplicate_planning', 'syet_duplicate_planning');
...
//add_action('wp_ajax_save_refresh_planning', 'save_refresh_planning');
